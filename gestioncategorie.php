...
function afficherCategories($tabCategNV1, $tabCategNV2, $tabCategNV3,$listeCategorie) {
    ...
    foreach ($tabCategNV1 as $idCategorie1) {
        $affichage .= '<a href="gestioncategorie.php?idCategorie=' . $idCategorie1 . '"><li>'.getNomCategorie($idCategorie1,$listeCategorie).'</a>';
        $affichage .= '<ul>';

        // Parcourir les catégories de niveau 2
        foreach ($tabCategNV2 as $idCategorie2 => $parent) {
            if ($parent == $idCategorie1) {
                $affichage .='<a href="gestioncategorie.php?idCategorie=' . $idCategorie2 . '"><li>'.getNomCategorie($idCategorie2,$listeCategorie).'</a>';
                $affichage .= '<ul>';

                // Parcourir les catégories de niveau 3
                foreach ($tabCategNV3 as $idCategorie3 => $parent2) {
                    if ($parent2 == $idCategorie2) {
                        $affichage .= '<a href="gestioncategorie.php?idCategorie=' . $idCategorie3 . '"><li>'.getNomCategorie($idCategorie3,$listeCategorie).'</a>';
                    }
                }
                $affichage .= '</ul>';
                $affichage .= '</li>';
